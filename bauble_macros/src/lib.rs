...
use quote::quote;
use syn::{
    parse_macro_input, AttrStyle, Attribute, Data, DeriveInput, Error, Expr, Fields, ImplGenerics,
    Index, Token, Type, WhereClause,
};
    ...
    let match_value = if flatten {
        ...
        match ast.data {
            ...
            Data::Enum(data) => {
                ...
                let variant_convert = data.variants.into_iter().map(|variant| {
                    let ident = &variant.ident;
